...
volatile bool veDirectBinaryBusy = false;
portMUX_TYPE veDirectBinaryMux = portMUX_INITIALIZER_UNLOCKED;
}
...
static byte calculateVEdirectTextChecksum(const String& output) {
  byte checksum = 0;
  const size_t outputLength = output.length();

  for (size_t i = 0; i < outputLength; ++i) {
    checksum += static_cast<byte>(output.charAt(i));
  }

  return static_cast<byte>(0 - checksum);
}
...
void sendVEdirect(){
  ...
  byte checksum = calculateVEdirectTextChecksum(vedirectOutput);
  ...
  checksum = calculateVEdirectTextChecksum(vedirectOutput);

  Serial1.print(vedirectOutput);                      // Wirite data
  Serial1.write(checksum);                            // Write checksum
}

static bool parseVEdirectHexNibble(char value, byte& nibble) {
  if (value >= '0' && value <= '9') {
    nibble = value - '0';
    return true;
  }
...
void sendBinaryValue(String type, int value){
  ...
  const size_t outputLength = valueOut.length();
  if (outputLength < 2 || (outputLength % 2) != 0 || valueOut.charAt(0) != ':') {
    DebugPrintln(1, "Invalid VE.direct binary frame length");
    return;
  }

  // Calculate checksum
  for (size_t i = 0; i + 1 < outputLength; i += 2) {
    byte highNibble = 0;
    byte lowNibble = 0;
    if ((i > 0 && !parseVEdirectHexNibble(valueOut.charAt(i), highNibble)) ||
        !parseVEdirectHexNibble(valueOut.charAt(i + 1), lowNibble)) {
      DebugPrintln(1, "Invalid VE.direct binary frame character");
      return;
    }
    checksum += static_cast<byte>((highNibble << 4) | lowNibble);
  }
...
namespace {
void veDirectBinaryTask(void *parameter) {
  (void)parameter;
  for (;;) {
    ulTaskNotifyTake(pdTRUE, portMAX_DELAY);
    sendVEdirectBinary();
    portENTER_CRITICAL(&veDirectBinaryMux);
    veDirectBinaryBusy = false;
    portEXIT_CRITICAL(&veDirectBinaryMux);
  }
}
}

bool queueVEdirectBinary() {
  if (veDirectBinaryTaskHandle == nullptr) {
    if (xTaskCreate(veDirectBinaryTask, "veDirectBinary", 4096, nullptr, 1, &veDirectBinaryTaskHandle) != pdPASS) {
      veDirectBinaryTaskHandle = nullptr;
      DebugPrintln(1, "Unable to create VE.direct background task");
      return false;
    }
  }

  portENTER_CRITICAL(&veDirectBinaryMux);
  if (veDirectBinaryBusy) {
    portEXIT_CRITICAL(&veDirectBinaryMux);
    return false;
  }
  veDirectBinaryBusy = true;
  portEXIT_CRITICAL(&veDirectBinaryMux);
  xTaskNotifyGive(veDirectBinaryTaskHandle);
  return true;
}

bool isVEdirectBinaryBusy() {
  portENTER_CRITICAL(&veDirectBinaryMux);
  const bool busy = veDirectBinaryBusy;
  portEXIT_CRITICAL(&veDirectBinaryMux);
  return busy;
}
